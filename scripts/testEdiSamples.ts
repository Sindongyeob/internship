...
// warpImage()가 실패 시 console.error(JSON.stringify(...))로 남기는 로그를 터미널과
// 파일(LOG_PATH) 양쪽에 남기기 위해 console.error를 가로챈다. inputPath만 담겨 한 줄이
// 짧으므로(예전엔 base64가 포함돼 길었다) 잘라낼 필요 없이 그대로 both에 적는다.
...
// 결과를 results/edi/에 저장한다. 실패 로그(JSON, inputPath 포함)는 results/edi-run.log에 남는다.
async function main() {
    ...
    try {
        ...
        const methodCounts: Record<'deskew' | 'original', number> = {
            deskew: 0,
            original: 0,
        };
        let totalPages = 0;
        ...
        for (const file of files) {
            ...
            for (let pageNum = 1; pageNum <= doc.length; pageNum++) {
                ...
                // 실패(method: 'original')한 경우는 임시 PNG를 지우지 않는다 — 실패 로그의
                // inputPath가 이 경로를 가리키므로, 나중에 로그만 보고 원본을 열어볼 수 있어야 한다.
                if (method !== 'original') {
        ...
        console.log(
            `완료: deskew ${methodCounts.deskew}개 / original ${methodCounts.original}개 (총 ${totalPages}페이지)`
        );
        console.log(`실패(original) 로그(JSON, inputPath 포함) 전체 내용: ${LOG_PATH}`);
    } finally {
        restore();
        await close();
    }
